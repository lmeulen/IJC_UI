 ...
 */
public class NieuweStandModel extends AbstractTableModel {
    ...
    private String[] columnNames = {" ", "Naam", "Afk", "Punten"};

    public NieuweStandModel() {
        this(0, null);
    }

    public NieuweStandModel(int groepID, JComponent component) {
        super();
    ...
    public int getRowCount() {
        if (controller.getResultaatGroepByID(groepID) != null) {
            return controller.getResultaatGroepByID(groepID).getSpelers().size();
        } else {
            return 0;
        }
    }
    ...
    public Object getValueAt(int row, int col) {
        Speler speler = controller.getResultaatGroepByID(groepID).getSpelers().get(row);
        switch (col) {
            case 0:
    ...
    public Object getToolTip(int row, int col) {
        if (row < controller.getResultaatGroepByID(groepID).getSpelers().size()) {
            Speler speler = controller.getResultaatGroepByID(groepID).getSpelers().get(row);
